...
describe('INV-27 sortOrder 与新建追加末尾', () => {
  it('quickAddTask 追加到同级组末尾;capture 逐条递增', () => {
    const base = snapshot({
      tasks: [
        task({ id: 't1', bucket: 'inbox', sortOrder: 0 }),
        task({ id: 't2', bucket: 'inbox', sortOrder: 1 }),
      ],
    });
    expect(nextSortOrder(base, { bucket: 'inbox', projectId: null, parentTaskId: null })).toBe(2);
    const r = quickAddTask(base, deps(), { title: '新' });
  ...
  it('addSubtask 追加到父的子任务组末尾(独立于顶层组)', () => {
    const base = snapshot({
      tasks: [task({ id: 'p' }), task({ id: 's1', parentTaskId: 'p', sortOrder: 0 })],
    });
...
describe('INV-27 reorderTask 重排与嵌套', () => {
  const base = () =>
    snapshot({
      tasks: [
        task({ id: 'a', bucket: 'inbox', sortOrder: 0 }),
        task({ id: 'b', bucket: 'inbox', sortOrder: 1 }),
        task({ id: 'c', bucket: 'inbox', sortOrder: 2 }),
      ],
    });
  ...
  it('nextRootSortOrder:孤儿子任务根(父已 done)计入顶层展示集,顶层追加落到其后', () => {
    ...
    const snap = snapshot({
      tasks: [
        task({ id: 'r', bucket: 'inbox', sortOrder: 0 }),
        task({ id: 'p', bucket: 'inbox', status: 'done', sortOrder: 1 }),
        task({ id: 's', bucket: 'inbox', parentTaskId: 'p', sortOrder: 7 }),
      ],
    });
    ...
    const r = quickAddTask(snap, deps(), { title: '新' });
    if (isUsecaseError(r)) throw new Error(r.error);
    const cmd = r.commands[0]!;
    if (cmd.kind !== 'createTask') throw new Error('expected createTask');
  ...
  it('可相对孤儿根(父已 done)重排:beforeId=孤儿根不再误报"不在同级组"', () => {
    ...
    const snap = snapshot({
      tasks: [
        task({ id: 'r1', bucket: 'inbox', sortOrder: 0 }),
        task({ id: 'p', bucket: 'inbox', status: 'done', sortOrder: 1 }),
        task({ id: 's', bucket: 'inbox', parentTaskId: 'p', sortOrder: 7 }),
      ],
    });
  ...
  it('孤儿根顶层重排保留 done 父链接(父 reopen 后回归,INV-26 契约)', () => {
    const snap = snapshot({
      tasks: [
        task({ id: 'p', bucket: 'inbox', status: 'done', sortOrder: 0 }),
        task({ id: 's', bucket: 'inbox', parentTaskId: 'p', sortOrder: 0 }),
        task({ id: 'r', bucket: 'inbox', sortOrder: 1 }),
      ],
    });
    ...
    const nested = snapshot({
      tasks: [
        task({ id: 'a', bucket: 'inbox', sortOrder: 0 }),
        task({ id: 'sa', bucket: 'inbox', parentTaskId: 'a', sortOrder: 0 }),
      ],
    });
  ...
  it('拖成子任务时项目一致:同项目内可嵌套', () => {
    const snap = snapshot({
      ...
      tasks: [
        ...
        task({ id: 'b', projectId: 'p1', bucket: 'project' }),
      ],
    });
