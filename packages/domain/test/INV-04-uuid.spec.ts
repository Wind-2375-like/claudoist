...
import { addContext } from '../src/usecases/contexts';
...
describe('INV-04 所有 usecase 创建的实体 id 均来自 deps.idGen 且互不相同', () => {
  it('跨 9 个创建型 usecase 用同一 seqIdGen:id 形态正确、无重复', () => {
    const home = ctx({ id: 'c-home', name: '@home', sortOrder: 0 });
    const phone = ctx({ id: 'c-phone', name: '@phone', sortOrder: 1 });
    const w = waitingFor({ id: 'w1', description: '报价单', delegatedTo: '老王' });
    const snap = snapshot({ contexts: [home, phone], waiting: [w] });
    const d = deps(); // seqIdGen('id') → id-1, id-2, …

    const ids: string[] = [];
    ids.push(...ok(captureToInbox(snap, d, { texts: ['想法一', '想法二'] })).createdIds);
    ids.push(ok(quickAddTask(snap, d, { title: '买牛奶', contextId: 'c-home' })).taskId);
    ...
    ids.push(ok(addContext(snap, d, { name: 'gym' })).contextId);
