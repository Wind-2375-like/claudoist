...
| 日期 | 变更 |
|---|---|
| 2026-09-02 | **D-41 Today 拍平:整个列表都能拖**(用户裁决「我需要 today 的顺序能拖, 不是按照时间来排!不要『带时间的任务在日历段按时刻排;要换位置请改它的时间』」)。删掉 D-40 初版的两段模型与那条拒绝理由,`dayOrder` 作用于 Today 的每一行,带时间的日历块也能拖到任意位置。接受的代价明写进 INV-38.1:**Today 的先后是「我打算先做哪件」,Calendar 的先后是「几点发生」,从此允许两者给出不同答案** —— Calendar 的 block 仍画在它的时刻上(一个把 15:00 的会画在 09:00 位置的日历是坏掉的日历),界面**不为此加任何提示**(用户明知代价才要的,再弹窗解释就是骚扰)。连带三项:① 失效规则去掉「改时刻」—— 没有段了,把会议从 15:00 挪到 16:00 不改变它是不是今天的事,Google 那边改个会议时间就打乱你今早排的顺序是最难联系起因果的一类失序;`updateTask` / `externalSync` / `pushSync` 三处写入口同步收紧为只判计划日。② 派生序保持扁平化之前的显示序**逐字不变**,因此**无需迁移**:两段时代排过的 `dayOrder` 只落在无时刻行上,扁平后它们仍在前、日历块仍以 null 垫底,老用户升级当天看不出任何变化(spec 有专测)。③ 渲染层删掉 `todaySortable` 标记与列表级兜底落点 —— 每行都收落点之后,那个兜底只会吞掉「瞄着某一行、差几像素没落上」的手势并把任务甩到末尾。466 测试 |
| 2026-09-01 | **v0.1.2 排序加固:设计面板 + 对抗审查共 8 条落地**。设计面板(3 提案 × 3 评审)三条:① 手动位置失效改判「值真的变了」而非「key 在不在 patch 里」—— 时间编辑器一次发 `{startTime, durationMinutes, timeZone}`、详情页「今天」按钮在已是今天时照写一遍、Google 每轮回传同值 patch,按 key 判会让这些与分段无关的编辑静默抹掉用户排好的序;② `moveTask` 挪进 someday/reference 清位(否则几周后挪回来会带着早被别人占用的号码插进队伍中间);③ **INV-28.2 条文修订**:Calendar 全天段比较器从 `createdAt` 改为同一个 `byDayOrder` —— 原条文的理由(sortOrder 只在同级组内有定义)对 `dayOrder` 不成立,继续用 createdAt 会让「Today 拖完切到 Calendar」出现两个答案。对抗审查(32 agent)另修五条:④ `pushSync.applyPulledEvents` 是第二个绕过 `updateTask` 的写入口,漏了同款失效(在 Google 里把 block 拖成定时再拖回全天 → 带着旧 dayOrder 插回队首);⑤ 镜像复活分支同理(不判值,直接回队尾);⑥ **agent `list_today` 并入 `todayList`** —— 它曾是 Today 排序的第三份实现、完全不读 dayOrder,后果是用户拖到第一位的那条在 agent 眼里还在中间,它会照着自己的顺序勾完成,**错的行会被真的改掉**;⑦ `ul` 兜底落点会吞掉「差几像素没落到行上」的拖拽(行间 2px margin、内联编辑卡片)把任务甩到段末且全程没有落点线,改为只在确实落于最后一条可排序行下方时才生效;⑧ 未定时段为空时(今天只有日历事件)拖定时行完全静默,能否看到 domain 备好的拒绝理由竟取决于「今天恰好有没有别的全天任务」。另把 `useReorderDrag` 的落点换算与「这一下会不会真的改变顺序」收成一份(`resolve`),hint 与 onDrop 共用 —— 否则会「画了插入线、松手却什么都没发生」。461 测试 |
| 2026-08-29 | **INV-37/D-39 项目手动排序 + INV-38/D-40 Today 手动排序**(用户诉求「project 的顺序可以挪动、today 的顺序也可以挪动」)。项目:`sortOrder`(迁移 v19 按 created_at 回填)+ `reorderProject`,侧栏与 My Projects 都能拖,四处显示统一走 `byProjectSort`。Today:两段模型 —— 未定时段可拖(`dayOrder`,迁移 v20 不回填),定时段锚时刻不可拖(它同时画在 Calendar 上,手动挪会让两视图矛盾);拖拽整段 materialize,顺序没变则零命令;失效规则只有一条(改日期或时刻 → 回队尾),`externalSync` 自带同款。顺带修一处既有隐患:Today 的排序原本桌面与 CLI **各排一遍**、靠注释写「改动需同步」,现收拢进 `rules/todayList.ts` 一份(INV-20.6)。设计经三方案独立提案 + 三维评审(工作流),三份提案独立收敛到同一架构 |
...
| 2026-08-12 | **用户一轮反馈修复(6 条)+ D-35 项目软删除**。两条是真 bug:① externalSync 的"复活"分支设 active 却不清 completedAt,产出非法组合,表现为"画成已完成、点撤销却说只能重开已完成的行动"且粘着 —— 修 usecase + VM 加权威 `done` 字段 + 迁移 v15 修存量;② 右键菜单被裁不是层叠而是 **overflow 裁剪**,收成 portal 组件。另修 IME 组字回车误提交(全应用 12 处)、5 处日期改原生选择器、日历已完成块制造死区、列表左边界不一致。**INV-34/D-35 项目软删除**:`contents` 由用户显式二选且无默认值(活跃任务绝不能留在已删项目里),⌘K + 只读视图 + 恢复按钮构成可发现的回收路径,收紧 6 处"把已删项目当活项目"的守卫,迁移 v16 重建 projects 表(用户库副本实测:行数/外键/store 往返/CHECK 全过) |
