...
2. **两段**:`startTime === null` = **全天段**;`startTime !== null` = **定时段**(按 startTime、createdAt 兜底)。**两段都不按 `sortOrder` 排** —— sortOrder 只在同级组(bucket+projectId+parentTaskId)内有定义(INV-27),跨容器比较会让别处的拖拽排序改变日历顺序、并与 Today 相互矛盾;全天段与 Today **一致**,即按 `byDayOrder`(手排过的在前,其余按创建序兜底,INV-38)。**2026-09-01 修订**:原文写死 `createdAt`,理由是「sortOrder 只在同级组内有定义」——那个理由对 `sortOrder` 成立、对 `dayOrder` **不成立**,`dayOrder` 就是为跨容器的 Today 列表定义的。继续用 `createdAt` 的后果是:在 Today 拖完、切到 Calendar,今天的全天条还是旧序 —— 同一天的同一批任务,两个视图两个答案。**2026-09-02 补(D-41)**:Today 拍平之后 `dayOrder` 也落在定时行上了,但**定时段仍按时刻画,不跟 Today 的手动序** —— 一个把 15:00 的会画在 09:00 位置的日历是坏掉的日历。于是「Today 与 Calendar 对同一批定时任务给出不同先后」成为**有意为之**的、用户明确买单的代价(INV-38.1),界面上不为此加任何提示。全天段仍与 Today 同序,因为那里没有时刻要遵守。
...
#### INV-38 Today 手动排序(D-40 立,D-41 拍平,2026-09-02)

**一条扁平列表,每一行都能手动拖**(`rules/todayList.ts` —— Today 列表的**唯一口径**,桌面 / CLI / agent `list_today` 三处共用,INV-20.6)。agent 那一份曾经自排一遍,后果是用户拖到第一位的那条在 agent 眼里还在中间 —— 它会照着自己看到的顺序回答「第一件事是什么」、按序号勾完成,**错的行会被真的改掉**。

| | |
|---|---|
| **成员** | 计划到今天或更早的活跃任务 ∪ 未排期的过期截止项(someday/reference 不入) |
| **排序** | `dayOrder` 升序在前;`null`(没拖过)按派生序接在后面 |
| **可手动拖** | **每一行,含带时间的日历块** |

1. **带时间的行也能拖(D-41,用户裁决)**。初版曾把 Today 分成「未定时段可拖 / 定时段锚时刻不可拖」,拖定时行会弹「带时间的任务在日历段按时刻排;要换位置请改它的时间」。理由是定时任务同时画在 Calendar 上,在 Today 手动挪会让两个视图对同一批任务给出矛盾的先后。**用户明确推翻**(原话:「我需要 today 的顺序能拖, 不是按照时间来排」),并接受这个代价 ——

   > **Today 的先后是「我打算先做哪件」,Calendar 的先后是「几点发生」。这是两个问题,从此允许它们给出不同的答案。**

   Calendar 的 block 当然仍然画在它的时刻上:一个把 15:00 的会画在 09:00 位置的日历是坏掉的日历。界面上**不为此加任何提示** —— 用户是明知代价才要的,再弹窗解释就是骚扰。
2. **`Task.dayOrder`(可空整数,独立于 `sortOrder`)**:`sortOrder` 是「任务在自己容器里的位置」,Today 却把不同容器的任务混在一起,两者不可比,故独立一列(迁移 v20,**不回填** —— 全 NULL = 没人排过)。`null` = 没排过,按派生序垫底(与 INV-27.1 新建追加末尾同规)。
3. **派生序 = 扁平化之前的默认显示序,逐字保留**:先无时刻的计划任务(过期的更靠前)、再未排期的过期截止项、最后是日历块按(计划日 → 时刻)。**不改成「全部按时刻穿插」**:那会让所有老用户升级当天看到列表自己重排了一遍,而他们什么都没做。想要什么顺序自己拖 —— 这次给的正是这个能力。同理,D-41 上线**不需要迁移**:两段时代排过的 `dayOrder` 只落在无时刻行上(0..N-1),扁平后它们仍排在前面、日历块仍以 `null` 垫底,显示序与升级前逐字相同。
4. **拖拽 = 整列表 materialize 0..N-1**:只写被拖的那一条不行 —— 别人都还是 `null`(垫底),它会跳到最前面,而用户明明把它放在中间。**顺序没变则零命令**。
5. **手动位置失效只有一条规则:不再是今天这张列表的成员,位置就作废**(`dayOrder := null`,回队尾)。两种触发:**改计划日**(换了一天就是换了一张列表)、`moveTask` 挪进 someday/reference(整个离开 Today)。完成 / reopen / 软删 / 恢复 **都不清**。

   **改时刻不清**(D-41 起)。扁平化之后没有「段」了,把会议从 15:00 挪到 16:00 不改变它是不是今天的事,凭什么把用户排好的位置作废?Google 那边改个会议时间就把你今早排的顺序打乱一格,是最难联系起因果的一类失序。

   **判「值真的变了」,不是判「这个 key 在 patch 里」**。调用方常把一组字段整包回传:详情页的「今天」按钮在任务已经是今天时照样写一遍 `scheduledDate`,Google 轮询每轮都回传一份同值 patch。按 key 判会让这些什么都没改的编辑静默抹掉用户排好的序,而用户根本无从把两件事联系起来。**绕过 `updateTask` 直接构造 updateTask 命令的写入口都要自带一份同款失效**,目前有三处:`externalSync` 的常规更新分支(判值)、`externalSync` 的**镜像复活**分支(不判值 —— 它在外面消失过一段时间,那一段早被重排,旧号码要么撞别人要么是孤号,回队尾才诚实)、`pushSync.applyPulledEvents`(D-26 专用日历回同步:在 Google 里把 block 拖到别的一天再拖回来,漏了就会带着旧 `dayOrder` 插回队首)。
6. **循环生成的下一次不继承 `dayOrder`**(`{...task}` 展开会带着旧值,必须显式清):新的一次是新面孔,回队尾。
7. **跨零点用主进程时钟**:`reorderTodayTask` 取 `deps.clock.today()`,不接受渲染层传日期 —— 渲染层缓存的 `data.today` 跨零点后可能是昨天(既有的「推迟到明天」已踩过这个坑)。
8. **渲染层没有列表级的兜底落点**:每一行都收落点,任何位置都能用行内落点表达(含「放到最末尾」= 落在最后一行的下半区)。落在行间 2px 缝隙或内联编辑卡片上的手势因此**什么都不做** —— 那是「瞄着某一行、差几像素没落上」,把任务甩到列表末尾是最坏的回答,而且全程没有任何落点线预告过。
9. **与升级前的可见差异**:未排期的过期截止项(due)从「列表最后」移到了无时刻任务之后、日历块之前。这是有意的 —— 过期的东西更该被看见,而且现在用户可以自己拖。

**代价(明写)**:① 一次拖拽会把当时列表的**全部**行定死为 0..N-1,包括用户没碰过的那些;此后它们不再随派生序自动上浮(手动排序的固有价格,INV-27.2 已经付过一次)。② Today 与 Calendar 对同一批定时任务可以给出不同先后 —— 见第 1 条,这是用户明确买单的。

**验收**:`INV-38-today-order.spec.ts`(默认序逐字不变、someday/已完成/未来计划不入、materialize 整列表、**带时间的会议可拖到任意位置**、**落点是会议不再被拒**、**拖完顺序稳定不被时刻拽回**、拖到末尾/自身/原位、过期截止项可拖、新任务垫底、改期清位置、**改时刻不清**、改别的字段不清、同值整包回传不清、挪进 someday 清、**升级兼容:两段时代的 dayOrder 照旧生效**)+ `INV-28.2` 同序用例(在 Today 拖完今天的全天条跟着变;**日历定时段仍按时刻画,与 Today 故意不同**)+ `INV-30-push-sync.spec.ts`(回同步改期清位、**只改时刻不清位**、无变化不发命令)+ `INV-29-external-mirror.spec.ts`(镜像复活清位)+ `read-tools.spec.ts`(agent `list_today` 与 `todayList` 逐字对拍)。
...
| D-40 | Today 顺序完全派生,用户没法安排「今天先做哪件」 | **Today 手动排序(同上诉求,INV-38)**:两段 —— 未定时段可手动拖(`Task.dayOrder`,迁移 v20,不回填),定时段锚在时刻上不可拖(它同时画在 Calendar 上,手动挪会让两个视图对同一批任务给出矛盾顺序)。顺带把 Today 的排序口径从「桌面与 CLI 各排一遍、靠注释说『改动需同步』」收拢进 `rules/todayList.ts` 一份(INV-20.6)。失效规则只有一条:不再是同一个未定时段的成员(改了日期/时刻、或挪进 someday),手动位置作废回队尾 —— 一条才记得住;**判值真的变了、不判 key 在不在 patch 里**(整包回传的同值编辑不能抹掉用户排好的序)。同时把 INV-28.2 的全天段比较器从 `createdAt` 改成同一个 `byDayOrder`,否则拖完 Today 切到 Calendar 会看到两个答案 | ✅ 已定 |
| D-41 | Today 分两段,带时间的行拒绝手动拖(D-40 初版) | **Today 拍平:整个列表都能拖(2026-09-02 用户裁决「我需要 today 的顺序能拖, 不是按照时间来排」)**。删掉两段模型与那条拒绝理由;`dayOrder` 作用于全部行。接受的代价明写:**Today 的先后是「我打算先做哪件」,Calendar 的先后是「几点发生」,从此允许两者不同** —— Calendar 的 block 仍画在它的时刻上(否则日历就坏了),界面不为此加提示(用户明知代价才要的)。连带:失效规则里**去掉「改时刻」**(没有段了,改钟点不改变「是不是今天的事」),三处写入口同步;派生序保持扁平化之前的顺序逐字不变,故**无需迁移**、老用户升级当天看不出变化;渲染层去掉 `todaySortable` 标记与列表级兜底落点(每行都收落点之后它只会吞掉「差几像素没落上」的手势) | ✅ 已定 |
